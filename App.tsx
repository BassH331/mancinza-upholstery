...
export default function App() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => entry.isIntersecting && setCurrentSection(entry.target.id));
      },
      { threshold: 0.6 }
    );
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="fixed inset-y-0 right-4 flex flex-col justify-center z-50 gap-4">
        {sections.map(({ id, icon }) => (
          <button key={id} onClick={() => scrollToSection(id)} className="flex items-center gap-2 group" aria-label={id}>
            <span
              className={`w-3 h-3 md:w-4 md:h-4 rounded-full transition-all duration-300 ${
                currentSection === id ? "bg-emerald-900 scale-125 shadow-md" : "bg-gray-400 opacity-50 group-hover:opacity-80"
              }`}
            />
            <span className={currentSection === id ? "text-green-900" : "text-gray-400 group-hover:text-teal-700"}>{icon}</span>
          </button>
        ))}
      </div>
